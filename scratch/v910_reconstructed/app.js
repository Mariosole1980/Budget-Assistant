...
const state = {
  ...
  notifications: [],
  hideAmounts: localStorage.getItem('settings_hide_amounts') === 'true',
};
...
const TRANSLATIONS = {
  el: {
    ...
    settings_trash_title: 'Κάδος Ανακύκλωσης',
  ...
  en: {
    ...
    settings_trash_title: 'Recycle Bin',
...
function evaluateCalcBuffer(buf) {
  ...
  try {
    const sanitized = cleanBuf.replace(/[^0-9\.\-\+\*\/]/g, '');
...
function addNotification({ title, body, type = 'info', action = null }) {
...
function renderNotificationHistoryList() {
  const historyListEl = document.getElementById('notifications-history-list');
  ...
  if (notifs.length === 0) {
    historyListEl.innerHTML = `
      <div style="text-align: center; padding: 24px 12px; color: var(--text-muted); font-size: 13px;">
        ...
        ${state.lang === 'el' ? 'Δεν υπάρχει ιστορικό ειδοποιήσεων.' : 'No notification history.'}
      </div>
    `;
    return;
  }
  notifs.forEach(notif => {
    const card = document.createElement('div');
    card.style.cssText = 'background: var(--bg-card); border: 1px solid var(--border); border-radius: 12px; padding: 12px; margin-bottom: 8px; display: flex; flex-direction: column; gap: 4px;';
    const notifTime = notif.timestamp || notif.date || new Date().toISOString();
    const formattedTime = new Date(notifTime).toLocaleString(state.lang === 'el' ? 'el-GR' : 'en-US', {
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
      minute: '2-digit'
    });
    const typeBadge = notif.type === 'warning' ? '🔴 ' : (notif.type === 'recurring' ? '⏳ ' : '🔔 ');
    card.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <strong style="font-size: 13px; color: var(--text-primary); font-family: 'Outfit', sans-serif;">${typeBadge}${escapeHtml(notif.title)}</strong>
        <span style="font-size: 11px; color: var(--text-muted);">${formattedTime}</span>
      </div>
      <div style="font-size: 12.5px; color: var(--text-secondary); line-height: 1.4;">${escapeHtml(notif.body)}</div>
    `;
    historyListEl.appendChild(card);
  });
}
window.renderNotificationHistoryList = renderNotificationHistoryList;
...
window.clearNotificationHistory = clearNotifications;

function toggleExpenseAlertSetting(checked) {
  localStorage.setItem('settings_expense_alert_enabled', checked ? 'true' : 'false');
  const row = document.getElementById('settings-expense-alert-limit-row');
  if (row) row.style.display = checked ? 'flex' : 'none';
}
window.toggleExpenseAlertSetting = toggleExpenseAlertSetting;

function changeExpenseAlertLimit(val) {
  const num = parseFloat(val);
  if (!isNaN(num) && num > 0) {
    localStorage.setItem('settings_expense_alert_limit', num.toString());
  }
}
window.changeExpenseAlertLimit = changeExpenseAlertLimit;

function checkRecurringAlerts() {
  const recurringEnabled = localStorage.getItem('settings_recurring_alerts_enabled') !== 'false';
  if (!recurringEnabled || !state.recurringTemplates || state.recurringTemplates.length === 0) return;
  ...
  state.recurringTemplates.forEach(tpl => {
    if (!tpl.active && tpl.active !== undefined) return;
    const tplDay = parseInt(tpl.day_of_month || tpl.day || '1', 10);
    if (tplDay === tomorrowDay) {
      const cacheKey = `notified_rec_${tpl.id}_${tomorrowStr}`;
      if (!localStorage.getItem(cacheKey)) {
        localStorage.setItem(cacheKey, 'true');
        const lang = state.lang || 'el';
        const title = lang === 'el' ? '⏳ Προγραμματισμένη Πληρωμή (Αύριο)' : '⏳ Upcoming Recurring Payment (Tomorrow)';
        const amountStr = formatCurrency ? formatCurrency(tpl.amount) : tpl.amount;
        const catName = tpl.note || (getCategoryDisplayName ? getCategoryDisplayName(tpl.category) : tpl.category);
        const body = lang === 'el'
          ? `Αύριο είναι προγραμματισμένη η πληρωμή: ${catName} (${amountStr}€).`
          : `Upcoming payment scheduled for tomorrow: ${catName} (${amountStr}€).`;
        addNotification({ title, body, type: 'recurring', action: { type: 'open_transactions' } });
      }
    }
  });
}
window.checkRecurringAlerts = checkRecurringAlerts;
  ...
  if (state.isSupabaseEnabled && state.supabaseClient && state.currentUser) {
    try {
      ...
      */
      ...
      // Load local trash transactions & sync with Supabase deleted_transactions table
      try {
        const trashStr = localStorage.getItem('deleted_transactions_trash');
        state.trashTransactions = trashStr ? JSON.parse(trashStr) : [];
      } catch (e) {
        state.trashTransactions = [];
      }

      // Fetch cloud trash transactions in background to keep recycle bin in sync
      if (state.supabaseClient && state.currentUser) {
        promiseTimeout(
          ...
          if (cloudTrash && cloudTrash.length > 0) {
            ...
            state.trashTransactions = Array.from(existingMap.values());
            ...
            const countEl = document.getElementById('sync-trash-count') || document.getElementById('trash-bin-count-val');
            if (countEl) countEl.textContent = state.trashTransactions.length;
          }
        }).catch(err => console.warn('Cloud trash fetch error:', err));
      }
...
function autoRecoverTemplatesFromHistory() {
  ...
  const recoverForKeywords = (keywords, defaultNote, preset = 'monthly') => {
    ...
    const cleanedNote = (tx.note && tx.note.trim()) ? tx.note.trim() : defaultNote;
    ...
    for (let m = 0; m <= currentMonthLimit; m++) {
      ...
      datesToProcess.forEach(dateString => {
        ...
        const duplicateExists = state.transactions.some(t => {
          ...
          // Content-based fallback: same date + amount + type + category + note + account_from
          if (tDate === dateString &&
              (parseFloat(t.amount) || 0).toFixed(2) === (parseFloat(template.amount) || 0).toFixed(2) &&
              t.type === template.type &&
              t.category === template.category &&
              (t.note || '').trim().toLowerCase() === (template.note || '').trim().toLowerCase() &&
        ...
        if (!duplicateExists) {
          let itemNote = template.note || '';
          if (itemNote && itemNote.match(/(\d+)\/(\d+)/)) {
            const match = itemNote.match(/(\d+)\/(\d+)/);
            const startInst = parseInt(match[1], 10);
            const totalInst = parseInt(match[2], 10);
            
            const startDateObj = template.startDate ? new Date(template.startDate) : (template.created_at ? new Date(template.created_at) : new Date(dateString));
            const currentDateObj = new Date(dateString);
            const monthDiff = (currentDateObj.getFullYear() - startDateObj.getFullYear()) * 12 + (currentDateObj.getMonth() - startDateObj.getMonth());
            const currentInst = Math.min(startInst + Math.max(0, monthDiff), totalInst);
            itemNote = itemNote.replace(/(\d+)\/(\d+)/, `${currentInst}/${totalInst}`);
          }
...
function saveTransactionOffline(transaction) {
  ...
  // Single Expense Alert check
  const expenseAlertEnabled = localStorage.getItem('settings_expense_alert_enabled') === 'true';
  const expenseAlertLimit = parseFloat(localStorage.getItem('settings_expense_alert_limit') || '500');
  if (expenseAlertEnabled && transaction.type === 'expense' && parseFloat(transaction.amount || 0) >= expenseAlertLimit) {
    const lang = state.lang || 'el';
    const title = lang === 'el' ? '⚠️ Υψηλή Δαπάνη!' : '⚠️ High Expense Alert!';
    const catName = transaction.note || (typeof getCategoryDisplayName === 'function' ? getCategoryDisplayName(transaction.category) : transaction.category);
    const amountStr = typeof formatCurrency === 'function' ? formatCurrency(transaction.amount) : transaction.amount;
    const limitStr = typeof formatCurrency === 'function' ? formatCurrency(expenseAlertLimit) : expenseAlertLimit;
    const body = lang === 'el'
      ? `Καταχωρήθηκε έξοδο ${amountStr}€ (${catName}) που ξεπερνά το όριο των ${limitStr}€.`
      : `Logged expense ${amountStr}€ (${catName}) exceeding your limit of ${limitStr}€.`;
...
function deleteTransaction(id) {
  ...
  if (!state.trashTransactions || !Array.isArray(state.trashTransactions)) {
    try {
      const trashStr = localStorage.getItem('deleted_transactions_trash');
      state.trashTransactions = trashStr ? JSON.parse(trashStr) : [];
    } catch (e) {
...
function updateUI(options = {}) {
  ...
  const immediate = typeof options === 'object' && options !== null && !!options.immediate;
  const baseDelay = immediate ? 0 : (window._appJustResumed ? 700 : 150);
...
function _updateUIImpl() {
  ...
  const trashCountEl = document.getElementById('sync-trash-count') || document.getElementById('trash-bin-count-val');
  if (trashCountEl) {
  ...
  if (!anyModalOpen) {
    ...
    if (restoredScrollContainer) {
      ...
      if (window._appJustResumed && bgScrollTop !== null && parseInt(bgScrollTop, 10) > 0) {
...
function renderTransactionsTab() {
  ...
  const transScrollEl = document.querySelector('.trans-scroll-content');
  const innerSavedScrollTop = transScrollEl ? transScrollEl.scrollTop : 0;
  ...
  if (transScrollEl && innerSavedScrollTop > 0) {
    transScrollEl.scrollTop = innerSavedScrollTop;
  }
}
    ...
    if (el) {
      ...
      el.addEventListener('focus', () => {
        ...
        if (textInputs.includes(id)) {
          ...
          if (isIOS && !isKeyboardAlreadyActive) {
            // iOS: apply estimated keyboard height instantly BEFORE visualViewport fires
            document.documentElement.style.setProperty('--keyboard-height', '320px');
          }
undefined
          // Call the viewport updater immediately and with delays for accuracy
          if (typeof window._updateViewportHeight === 'function') {
            setTimeout(window._updateViewportHeight, 100);
            setTimeout(window._updateViewportHeight, 300);
            setTimeout(window._updateViewportHeight, 500);
          }
...
function openModal(id, { instant = false } = {}) {
  ...
  if (id === 'transaction-modal') {
    if (!isIOS) {
      ...
      // Apply initial safe padding so buttons don't hide behind Android navigation bar
      el.style.setProperty('padding-bottom', (56 / scale) + 'px', 'important');
...
function openEditTransactionModal(t, { instant = false } = {}) {
  // Ghost click guard: block opening if any modal was closed within the last 350ms
  if (!instant && Date.now() - (state.lastModalCloseTime || 0) < 350) {
    console.log('[openEditTransactionModal] Blocked ghost click open after modal close');
    return;
  }
...
function initSwipeToBack() {
  ...
  function triggerBackAction() {
    ...
    // 2. Close active modals in top-down stack order (most recently opened modal first)
    const activeModals = document.querySelectorAll('.modal-overlay.active, .tx-modal-overlay.active, .profile-sheet-overlay.active');
    if (activeModals.length > 0) {
      const topModal = activeModals[activeModals.length - 1];
      if (topModal.id === 'custom-date-picker-modal') {
  ...
  function finishDrag(committed) {
    ...
    if (committed && currentTabIdx > 0) {
      ...
      setTimeout(() => {
        // Clean up transient drag inline styles on both screens
        cleanupDragStyles(currScreen);
        cleanupDragStyles(pScreen);

        // Formally switch to the target previous tab synchronously
        const prevTabName = TAB_ORDER[currentTabIdx - 1];
...
function initSettingsFromStorage() {
  ...
  const screenshotBlockEnabled = localStorage.getItem('settings_screenshot_block_enabled') === 'true';
  ...
  if (screenshotBlockEnabled && window.Capacitor && window.Capacitor.isNativePlatform() && window.Capacitor.Plugins.PrivacyScreen) {
    window.Capacitor.Plugins.PrivacyScreen.enable().catch(console.warn);
  }
...
async function toggleScreenshotBlockSetting(checked) {
  try {
    localStorage.setItem('settings_screenshot_block_enabled', checked ? 'true' : 'false');
    if (window.Capacitor && window.Capacitor.isNativePlatform() && window.Capacitor.Plugins.PrivacyScreen) {
      if (checked) {
        await window.Capacitor.Plugins.PrivacyScreen.enable();
      } else {
        await window.Capacitor.Plugins.PrivacyScreen.disable();
      }
    } else {
      console.log('PrivacyScreen plugin not available or not on native platform.');
    }
  } catch (err) {
    console.error('Error toggling screenshot block:', err);
  }
}
...
async function triggerChangeEmail() {
  if (!state.currentUser || !state.supabaseClient) return;
  const lang = state.lang || 'el';
  const currentEmail = state.currentUser.email || '';
  const newEmail = prompt(
    lang === 'el'
      ? `Τρέχον Email: ${currentEmail}\n\nΕισάγετε το νέο σας Email:`
      : `Current Email: ${currentEmail}\n\nEnter your new Email:`,
    currentEmail
  );
  if (!newEmail || newEmail.trim() === '' || newEmail.trim().toLowerCase() === currentEmail.toLowerCase()) return;

  try {
    const { error } = await state.supabaseClient.auth.updateUser({ email: newEmail.trim() });
    if (error) throw error;
    alert(
      lang === 'el'
        ? 'Εστάλη σύνδεσμος επιβεβαίωσης στο νέο σας email. Παρακαλώ ελέγξτε τα εισερχόμενά σας.'
        : 'Confirmation link sent to your new email. Please check your inbox.'
    );
  } catch (err) {
    alert((lang === 'el' ? 'Σφάλμα αλλαγής email: ' : 'Error updating email: ') + (err.message || err));
  }
}
...
async function triggerChangePassword() {
  ...
  const newPass = prompt(
    lang === 'el'
      ? 'Εισάγετε τον νέο σας κωδικό πρόσβασης (τουλάχιστον 6 χαρακτήρες):'
      : 'Enter your new password (at least 6 characters):'
  );
  if (!newPass || newPass.trim().length < 6) {
    if (newPass !== null) {
      alert(lang === 'el' ? 'Ο κωδικός πρέπει να έχει τουλάχιστον 6 χαρακτήρες.' : 'Password must be at least 6 characters.');
    }
    return;
  }

  try {
    const { error } = await state.supabaseClient.auth.updateUser({ password: newPass.trim() });
    if (error) throw error;
    alert(
      lang === 'el'
    ...
    alert((lang === 'el' ? 'Σφάλμα αλλαγής κωδικού: ' : 'Error updating password: ') + (err.message || err));
  }
}

async function triggerGlobalSignOut() {
  ...
  const confirmed = await showConfirm(
    lang === 'el'
      ? 'Θέλετε να αποσυνδεθείτε από όλες τις συσκευές που χρησιμοποιούν αυτόν τον λογαριασμό;'
      : 'Do you want to sign out from all devices using this account?',
    lang === 'el' ? 'Αποσύνδεση από όλες' : 'Sign Out All Devices',
    '🔌'
  );
  if (!confirmed) return;
  ...
  try {
    state.isLoggingOut = true;
    try {
      await state.supabaseClient.auth.signOut({ scope: 'global' });
    } catch (e) {
      console.warn('Global signout error fallback:', e);
    }
    localStorage.clear();
    sessionStorage.clear();
    window.location.reload();
  } catch (err) {
    alert((lang === 'el' ? 'Σφάλμα αποσύνδεσης: ' : 'Error signing out: ') + (err.message || err));
  }
}
...
async function triggerDeleteAccount() {
  ...
  const confirmed1 = await showConfirm(
    lang === 'el'
      ? '⚠️ ΠΡΟΣΟΧΗ: Η διαγραφή του λογαριασμού σας είναι οριστική και αμετάκλητη!\n\nΌλα τα δεδομένα, οι συναλλαγές, οι κατηγορίες και οι ρυθμίσεις σας θα διαγραφούν οριστικά.\n\nΘέλετε να προχωρήσετε;'
      : '⚠️ WARNING: Account deletion is permanent and cannot be undone!\n\nAll your data, transactions, categories, and settings will be permanently erased.\n\nDo you wish to proceed?',
    lang === 'el' ? 'Οριστική Διαγραφή' : 'Delete Account',
    '⚠️'
  );
  ...
  const typedConfirm = prompt(
    lang === 'el'
      ? 'Για επιβεβαίωση, πληκτρολογήστε τη λέξη ΔΙΑΓΡΑΦΗ (ή DELETE):'
      : 'To confirm, type DELETE:'
  );
  if (!typedConfirm || (typedConfirm.trim().toUpperCase() !== 'ΔΙΑΓΡΑΦΗ' && typedConfirm.trim().toUpperCase() !== 'DELETE')) {
    alert(lang === 'el' ? 'Η επιβεβαίωση ακυρώθηκε.' : 'Confirmation cancelled.');
    return;
  }
  ...
  try {
    ...
    try {
      await state.supabaseClient.from('transactions').delete().eq('user_id', userId);
      await state.supabaseClient.from('categories').delete().eq('user_id', userId);
      await state.supabaseClient.from('accounts').delete().eq('user_id', userId);
      await state.supabaseClient.from('profiles').delete().eq('id', userId);
    } catch (dbErr) {
      console.warn('Database cleanup warning during account deletion:', dbErr);
    }
    ...
    localStorage.clear();
    sessionStorage.clear();
    alert(lang === 'el' ? 'Ο λογαριασμός σας διαγράφηκε επιτυχώς.' : 'Your account has been deleted.');
    window.location.reload();
  } catch (err) {
    alert((lang === 'el' ? 'Σφάλμα κατά τη διαγραφή: ' : 'Error during deletion: ') + (err.message || err));
  }
}

window.triggerChangeEmail = triggerChangeEmail;
window.triggerChangePassword = triggerChangePassword;
window.triggerGlobalSignOut = triggerGlobalSignOut;
...
function closeProfileSheet(e) {
  ...
  state.lastModalCloseTime = Date.now();

  // Prevent ghost clicks / click penetration on underlying elements (e.g. transaction list items)
  document.body.style.pointerEvents = 'none';
  setTimeout(() => {
    document.body.style.pointerEvents = '';
  }, 350);
...
// ============================================================
// ANDROID KEYBOARD: Use Capacitor Keyboard Plugin (most reliable)
// Listens to native keyboardWillShow/Hide events and directly
// sets padding-bottom on the transaction modal to push it above keyboard.
// ============================================================
(function setupAndroidKeyboard() {
  if (isIOS) return; // iOS handled differently

  function applyKeyboardPadding(kbHeight) {
    const txModal = document.getElementById('transaction-modal');
    if (txModal && txModal.classList.contains('active')) {
      const scale = 0.93; // Match body zoom
      // Add extra 60px physical buffer so it sits nicely below the inputs and clears the keyboard
      txModal.style.setProperty('padding-bottom', ((kbHeight + 60) / scale) + 'px', 'important');
      txModal.style.setProperty('box-sizing', 'border-box', 'important');
    }
  }
  ...
  function removeKeyboardPadding() {
    const txModal = document.getElementById('transaction-modal');
    if (txModal) {
      // Don't remove it entirely, keep a safe area (56px physical) so buttons don't get cut off by the Android navigation bar when closed
      const scale = 0.93;
      txModal.style.setProperty('padding-bottom', (56 / scale) + 'px', 'important');
      txModal.style.setProperty('box-sizing', 'border-box', 'important');
    }
  }

  // Try Capacitor Keyboard plugin
  function tryCapacitorKeyboard() {
    const KeyboardPlugin = window.Capacitor &&
                           window.Capacitor.Plugins &&
                           window.Capacitor.Plugins.Keyboard;
    if (!KeyboardPlugin) return false;
    KeyboardPlugin.addListener('keyboardWillShow', (info) => {
      // Small buffer (10px physical) to ensure it clears Android navigation bar edge cases
      applyKeyboardPadding((info.keyboardHeight || 300) + 10);
    });
  ...
  // Try window keyboard events (fired by Capacitor on Android)
  window.addEventListener('keyboardDidShow', (e) => {
    applyKeyboardPadding(e.keyboardHeight || 300);
  });
  window.addEventListener('keyboardDidHide', () => {
    removeKeyboardPadding();
  });

  // Try after deviceready
  document.addEventListener('deviceready', () => {
    tryCapacitorKeyboard();
  }, { once: true });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.visualViewport) {
    const updateViewportHeight = () => {
      ...
      let rawKeyboardHeight = isIOS ? (window.innerHeight - vvHeight) : (maxViewportHeight - vvHeight - offsetTop);
      
      // Keep keyboard height stable during keyboard animation to prevent modal shaking/collapsing
      if (document.body.classList.contains('keyboard-active')) {
        if (isIOS) {
          rawKeyboardHeight = Math.max(rawKeyboardHeight, 320);
        } else {
          rawKeyboardHeight = Math.max(rawKeyboardHeight, 280);
        }
      }
      
      // Scale heights inversely to counteract body { zoom: 0.93 }
      const scale = isIOS ? 1.0 : 0.93;
      const keyboardHeight = Math.max(0, rawKeyboardHeight) / scale;
      
      // --viewport-height: the visible area height (visual viewport) scaled
      document.documentElement.style.setProperty('--viewport-height', `${vvHeight / scale}px`);
      document.documentElement.style.setProperty('--viewport-offset-top', `${offsetTop / scale}px`);
      document.documentElement.style.setProperty('--keyboard-height', `${keyboardHeight}px`);

      // iOS: directly resize modal-content to visible area
      const txModal = document.getElementById('transaction-modal');
      if (txModal && txModal.classList.contains('active')) {
        const modalContent = txModal.querySelector('.modal-content');

        // iOS-specific: adjust overlay and content to match visual viewport exactly
        if (isIOS) {
          if (modalContent) {
            if (rawKeyboardHeight > 30) {
              const targetHeight = (vvHeight / scale);
              modalContent.style.setProperty('height', targetHeight + 'px', 'important');
              modalContent.style.setProperty('max-height', targetHeight + 'px', 'important');
            } else {
    ...
    updateViewportHeight();
    // Expose globally so focus handlers can trigger it immediately when keyboard opens
...
function adjustCustomDatePickerMonth(direction) {
  const currentDay = customDatePickerSelectedDate.getDate();
  customDatePickerViewingMonth.setDate(1);
  customDatePickerViewingMonth.setMonth(customDatePickerViewingMonth.getMonth() + direction);
  const maxDays = new Date(customDatePickerViewingMonth.getFullYear(), customDatePickerViewingMonth.getMonth() + 1, 0).getDate();
  const safeDay = Math.min(currentDay, maxDays);
  
  customDatePickerViewingMonth.setDate(safeDay);
  customDatePickerSelectedDate.setFullYear(customDatePickerViewingMonth.getFullYear());
  customDatePickerSelectedDate.setMonth(customDatePickerViewingMonth.getMonth());
  customDatePickerSelectedDate.setDate(safeDay);
  ...
  if (type === 'month') {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      btn.onclick = (e) => {
        ...
        const maxDays = new Date(customDatePickerViewingMonth.getFullYear(), m, 0).getDate();
        const safeDay = Math.min(currentDay, maxDays);
        
        customDatePickerViewingMonth.setDate(safeDay);
        customDatePickerSelectedDate.setFullYear(customDatePickerViewingMonth.getFullYear());
        customDatePickerSelectedDate.setMonth(m - 1);
        customDatePickerSelectedDate.setDate(safeDay);
    ...
    for (let y = startY; y <= endY; y++) {
      ...
      btn.onclick = (e) => {
        ...
        customDatePickerSelectedDate.setFullYear(y);
...
window.onSubscreenShow_preferences = function() {
  ...
  state.hideAmounts = hideAmountsEnabled;
...
window.onSubscreenShow_notifications = function() {
  const dailyEnabled = localStorage.getItem('settings_daily_reminder_enabled') === 'true';
  const dailyCb = document.getElementById('settings-daily-reminder');
  if (dailyCb) {
    dailyCb.checked = dailyEnabled;
    if (!dailyCb.dataset.listenerBound) {
      dailyCb.dataset.listenerBound = 'true';
      dailyCb.addEventListener('change', (e) => {
        const timeVal = localStorage.getItem('settings_daily_reminder_time') || '21:00';
        localStorage.setItem('settings_daily_reminder_enabled', e.target.checked ? 'true' : 'false');
        scheduleDailyReminder(e.target.checked, timeVal);
  ...
  if (dailyTimeInput) {
    ...
    if (!dailyTimeInput.dataset.listenerBound) {
      dailyTimeInput.dataset.listenerBound = 'true';
      dailyTimeInput.addEventListener('change', (e) => {
        localStorage.setItem('settings_daily_reminder_time', e.target.value);
        const enabled = localStorage.getItem('settings_daily_reminder_enabled') === 'true';
        scheduleDailyReminder(enabled, e.target.value);
      });
    }
  }

  const dailyTimeRow = document.getElementById('settings-daily-reminder-time-row');
  if (dailyTimeRow) dailyTimeRow.style.display = dailyEnabled ? 'flex' : 'none';

  const recurringEnabled = localStorage.getItem('settings_recurring_alerts_enabled') !== 'false';
  const recurringCb = document.getElementById('settings-recurring-alerts');
  if (recurringCb) {
    recurringCb.checked = recurringEnabled;
    if (!recurringCb.dataset.listenerBound) {
      recurringCb.dataset.listenerBound = 'true';
  ...
  const expenseEnabled = localStorage.getItem('settings_expense_alert_enabled') === 'true';
  const expenseCb = document.getElementById('settings-expense-alert');
  const expenseRow = document.getElementById('settings-expense-alert-limit-row');
  if (expenseCb) {
    expenseCb.checked = expenseEnabled;
    if (expenseRow) expenseRow.style.display = expenseEnabled ? 'flex' : 'none';
    if (!expenseCb.dataset.listenerBound) {
      expenseCb.dataset.listenerBound = 'true';
      expenseCb.addEventListener('change', (e) => {
        const isChecked = e.target.checked;
        localStorage.setItem('settings_expense_alert_enabled', isChecked ? 'true' : 'false');
        if (expenseRow) expenseRow.style.display = isChecked ? 'flex' : 'none';
      });
    }
  }

  const expenseLimit = localStorage.getItem('settings_expense_alert_limit') || '500';
  const expenseInput = document.getElementById('settings-expense-alert-limit');
  if (expenseInput) {
    expenseInput.value = expenseLimit;
    if (!expenseInput.dataset.listenerBound) {
      expenseInput.dataset.listenerBound = 'true';
      const saveLimit = (val) => {
        const num = parseFloat(val);
        if (!isNaN(num) && num > 0) {
          localStorage.setItem('settings_expense_alert_limit', num.toString());
      ...
      expenseInput.addEventListener('change', (e) => saveLimit(e.target.value));
      expenseInput.addEventListener('input', (e) => saveLimit(e.target.value));
    }
  }
...
function openTrashBinModal() {
  try {
    const trashStr = localStorage.getItem('deleted_transactions_trash');
    const localTrash = trashStr ? JSON.parse(trashStr) : [];
    const existingMap = new Map((state.trashTransactions || []).map(t => [String(t.id), t]));
  ...
  renderTrashBinList();
  openSettingsSubscreen('trash', 'settings_trash_title');
}

window.onSubscreenShow_trash = function() {
  renderTrashBinList();
};
...
function highlightGuideQuery(text, query) {
  ...
  const escaped = query.replace(/[.*+?^${}()|[\]\\]/g, '\\  setTimeout(() => {
    forceViewportReset();
  }, 800);
});');
  const regex = new RegExp(`(${escaped})`, 'gi');
    ...
    chapters: [
      {
        id: 'changelog',
        icon: 'fa-box-archive',
        title: '1. Έκδοση & Τι Νέο Υπάρχει (v891)',
        content: `
          <p><strong>Έκδοση Οδηγού:</strong> v891 | <strong>Συγχρονισμένη Έκδοση Εφαρμογής:</strong> v891</p>
          <div class="guide-feature-box">
            <h5 style="margin:0 0 6px; color:var(--primary);">✨ Τι νέο υπάρχει στην τελευταία έκδοση:</h5>
            <ul style="margin:0; padding-left:18px;">
              <li><strong>Προστασία Ghost Clicks (v891):</strong> Χειρουργικός μηχανισμός ClickGuard (350ms buffer) ώστε το πάτημα του 'X' στο Προφίλ μετά από επαναφορά της εφαρμογής να μην ανοίγει ποτέ υποκείμενη κίνηση.</li>
              <li><strong>Πλήρης Οδηγός Χρήσης (v890):</strong> Ενσωμάτωση ζωντανού δίγλωσσου οδηγού (Ελληνικά & Αγγλικά) στη σειρά Νομικά & Πληροφορίες.</li>
    ...
    chapters: [
      {
        id: 'changelog',
        icon: 'fa-box-archive',
        title: '1. Version & What\'s New (v891)',
        content: `
          <p><strong>Guide Version:</strong> v891 | <strong>Synchronized App Version:</strong> v891</p>
          <div class="guide-feature-box">
            <h5 style="margin:0 0 6px; color:var(--primary);">✨ What's new in the latest version:</h5>
            <ul style="margin:0; padding-left:18px;">
              <li><strong>Ghost Click Protection (v891):</strong> Surgical ClickGuard mechanism (350ms buffer) preventing profile close taps from triggering underlying transactions on app resume.</li>
              <li><strong>Full User Guide (v890):</strong> Embedded live bilingual user guide (Greek & English) under Legal & Info.</li>
...
function renderUserGuideContent(query = '') {
  ...
  const lang = window._userGuideLang || 'el';
